...
$popular_books_sql = "
    SELECT books.*, COUNT(borrowed_books.book_id) AS borrow_count 
    FROM books
      ...
      <?php while ($book = $popular_books_result->fetch_assoc()): ?>
        <div class="col">
          <div class="card book-card">
            <img src="<?= $book['cover_photo'] ?: 'https://via.placeholder.com/300x200'; ?>" class="card-img-top" alt="Book Cover">
            <div class="card-body">
              <h5 class="card-title"><?= htmlspecialchars($book['title']); ?></h5>
              <p class="card-text">Author: <?= htmlspecialchars($book['author']); ?></p>
              <p class="card-text"><small class="text-muted">Borrowed <?= $book['borrow_count']; ?> times</small></p>
            </div>
          </div>
        </div>
      <?php endwhile; ?>
